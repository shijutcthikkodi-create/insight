...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Attempt to auto-init on first interaction for laptops/desktops
  ...
  const handleRedirectToCard = useCallback((id: string) => {
    ...
    const scrollTask = () => {
      ...
      if (el && container) {
        ...
        const offsetPosition = elementPosition + (container.scrollTop || 0) - offset;
      ...
      if (data) {
              ...
              reconciledSignals.forEach(s => {
                ...
                if (!old) {
                  if (!isInitial && prevSignalsRef.current.length > 0) {
                    ...
                    notificationTitle = 'NEW EXECUTION ORDER';
                    notificationMsg = `${s.instrument} ${s.symbol} ${s.type} @ ₹${s.entryPrice}`;
                  }
                } else {
                  ALL_SIGNAL_KEYS.forEach(k => {
                    ...
                    if (JSON.stringify(newVal) !== JSON.stringify(oldVal)) {
                      ...
                      if (ALERT_TRIGGER_KEYS.includes(k as keyof TradeSignal)) {
                         ...
                         if (k === 'status' && newVal !== oldVal) {
                            notificationTitle = 'STATUS UPDATE';
                            notificationMsg = `${s.instrument} ${s.symbol}: ${oldVal} → ${newVal}`;
                            ...
                            notificationMsg = `${s.instrument}: ${newVal}`;
                         } else if (!notificationMsg) {
                            notificationMsg = `${s.instrument} ${s.symbol} updated in terminal.`;
                         }
                      }
                    }
                  });
        ...
        if (!isInitial && data.messages.length > 0) {
          ...
          if (latestAdminMsg && latestAdminMsg.id !== lastIntelIdRef.current) {
            ...
            notificationTitle = 'ADMIN BROADCAST';
            notificationMsg = latestAdminMsg.text;
          }
        }

        if (!isInitial) {
           if (signalChangeDetected || intelChangeDetected) {
             sendPushNotification(notificationTitle, notificationMsg);
           }
        ...
        const reconciledInsights = (data.insights || []).map(insight => {
          const { symbol, sentiment, protection, cmp: directCmp } = insight;
          const name = symbol.includes(',') ? symbol.split(',')[0].trim() : symbol;
          
          const isBull = ['BULLISH', 'BULL', 'BUY', 'UP', 'ACC'].includes(String(sentiment || '').trim().toUpperCase());
          const isBear = ['BEARISH', 'BEAR', 'SELL', 'DOWN', 'DIS'].includes(String(sentiment || '').trim().toUpperCase());
          
          const watchItem = data.watchlist.find(w => w.symbol.toUpperCase().includes(name.toUpperCase()));
    ...
    >
      ...
      <div className="fixed top-4 right-4 z-[100] flex flex-col items-end space-y-3">
        <div className={`bg-slate-900/95 backdrop-blur-md px-3 py-2 rounded-xl text-[10px] font-bold border shadow-2xl flex items-center ${connectionStatus === 'error' ? 'border-rose-500 bg-rose-950/20' : 'border-slate-800'}`}>
          ...
          <button onClick={() => sync(true)} disabled={connectionStatus === 'syncing'} className="p-1.5 rounded-lg bg-slate-800 text-blue-400 border border-blue-500/20 active:scale-90 transition-all">
        ...
        <div className="flex items-center space-x-2">
            <button 
              ...
              className={`p-4 rounded-full border shadow-2xl transition-all active:scale-90 ${notificationPermission === 'granted' ? 'bg-blue-500/20 border-blue-500/50 text-blue-400 shadow-blue-500/10' : notificationPermission === 'denied' ? 'bg-rose-500/20 border-rose-500/50 text-rose-400 shadow-rose-500/10' : 'bg-slate-800 border-slate-700 text-slate-500'}`}
              title={notificationPermission === 'granted' ? 'Push Notifications Active' : 'Enable Push Notifications'}
            >
              {notificationPermission === 'granted' ? <BellRing size={24} /> : notificationPermission === 'denied' ? <BellOff size={24} /> : <Bell size={24} />}
            </button>
